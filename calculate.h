...
static BigNumber* multiply(const BigNumber* A, const BigNumber* B) {
    ...
    int total_frac = fracA + fracB;

    for (int i = start; i < L; i++) {
        int pos_from_right = L - 1 - i;
        int in_fraction = (pos_from_right < total_frac);
