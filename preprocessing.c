...
//preprocess 함수에서 다음 문자 확인에 사용(괄호 곱셈 반영 알고리즘에 사용)
...
//중위식 -> 후위식 변환 
//단항 +/- 문제 해결: 식 시작 또는 '(' 또는 연산자 뒤에 오는 +/- 가 숫자에 붙으면 숫자 토큰으로 합침!! ex)(2*-3)
char* infix_to_postfix(const char *s) {
    ...
    for (int i = 0; i < len; ) {
        ...
        // 이항 연산자
        if (is_op(c)) {
            while (top >= 0 && opstack[top] != '(' && precedence(opstack[top]) >= precedence(c)) {
                out[j++] = opstack[top--];
                out[j++] = ' '; //스택안의 연산자가 새로운 연산자보다 우선순위가 더 높으면 pop
            }
            opstack[++top] = c; 
            prevType = 0;
            i++;
